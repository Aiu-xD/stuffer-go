...
func (s *Scraper) scrapeSourceWithTimeout(source string, proxiesChan chan<- types.Proxy) error {
	...
	// Create azuretls HTTP client with timeout for scraping
	var client httpclient.HTTPClientInterface
	azureClient, err := httpclient.NewAzureTLSClient(nil, 30*time.Second)
	if err != nil {
		s.Logger.Warn("Failed to create azuretls client for scraping, using standard HTTP client", map[string]interface{}{
			"source": source,
			"error": err.Error(),
			"correlation_id": correlationID,
		})
...
func (s *Scraper) testProxyWithURL(proxy *types.Proxy, testURL string) bool {
	...
	// Try to create azuretls client first
	azureClient, err := httpclient.NewAzureTLSClient(proxy, time.Duration(s.Config.ProxyTimeout)*time.Millisecond)
	if err != nil {
		// Fallback to standard HTTP client for different proxy types
		switch proxy.Type {
